...
class regClass
{
	...
	public function updateteacher(Teachers $Teachers,$db,$id)
	{
		...
		try
		{
			$query="UPDATE teachers SET firstnamet='".$Teachers->getName()."',lastnamet='".$Teachers->getlName()."',phonet='".$Teachers->getPHONE()."',mailt='".$Teachers->getEMAIL()."',price='".$Teachers->getmkhir()."',passwordt='".$Teachers->getpassword()."',about='".$Teachers->getabout()."' WHERE id_teachers=$id";
			$db->commit($query);
		}
		catch(PDOException $e)
		{
	...
	public function updatestudent(Student $Student,$db,$id)
	{
		...
		try
		{
			$query="UPDATE students SET firstnames='".$Student->getName()."',lastnames='".$Student->getlName()."',phones='".$Student->getPHONE()."',emails='".$Student->getEMAIL()."',passwords='".$Student->getpassword()."' WHERE id_students=$id";
			$db->commit($query);
		}
		catch(PDOException $e)
		{
